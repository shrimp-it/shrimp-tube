(() => {
  ...
  if (
    ...
    return //loggy("This demo is not supported on your browser. Your browser lacks support for `MediaStreamTrackProcessor` and `MediaStreamTrackGenerator`.");
  }
  if (!("BarcodeDetector" in window)) {
    return //loggy("This demo is not supported on your browser. Your browser lacks support for `BarcodeDetector`.");
  }
  ...
  document.querySelector(".barbutton").addEventListener("click", () => {
    document.querySelector("#loggy").innerHTML ="<li></li>";
    linkurlarray.forEach( url => loggy(url) )
  });
  const linkurlarray=[];
  const showLink = (urltext) => {
   //  alert("text")
    if(linkurlarray.indexOf(urltext)!== -1) {
  //alert("text") ;
      return
  //document.querySelector("fooout").innerHTML += "<li><a href='"+urltext+"' target='_blank' >"+urltext+"</a></li>";
 }  linkurlarray.push(urltext);
  }
  
  const loggy = (text) =>  {
    alert(text);
   document.querySelector("#loggy").innerHTML+="<li><a href='"+text+"' target='_blank'>"+text.substring(8, 28)+"..</a> </li>";
